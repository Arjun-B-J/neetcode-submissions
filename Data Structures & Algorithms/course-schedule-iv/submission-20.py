class Solution:
    def checkIfPrerequisite(self, numCourses: int, prerequisites: List[List[int]], queries: List[List[int]]) -> List[bool]:
        ...
        for crs in list(adj):  #the adj expands during dfs for the leaf causing runtime error, take snapshot and run
            dfs(crs)
        
        ans = []
        for pre,crs in queries:
            ans.append(crs in prereqHashMap and pre in prereqHashMap[crs])
        return ans